...
test('Diagnose sends one canonical bounded evidence packet as model case context', { concurrency: false }, async () => {
  let capturedPayload;
  const restores = [
  ...
  try {
    ...
    await withServer(router, async base => {
      ...
      assert.match(messages[0].content, /DIAGNOSTIC_EVIDENCE_PACKET_V1 as the sole structured case context/);
      assert.match(messages[1].content, /^DIAGNOSTIC_EVIDENCE_PACKET_V1:\n/);
