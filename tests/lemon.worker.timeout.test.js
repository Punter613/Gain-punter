...
test('short navigation probe gets resolver/setup grace without widening its crawl budget', () => {
  assert.equal(resolveHardTimeoutMs({ maxElapsedMs: 4500, hardTimeoutMs: 6500 }), 9500);
});

test('explicit larger hard timeout is preserved', () => {
  assert.equal(resolveHardTimeoutMs({ maxElapsedMs: 4500, hardTimeoutMs: 12000 }), 12000);
});
