...
function replaceOnce(file, oldText, newText) {
  ...
  if (!content.includes(oldText)) throw new Error(`Patch anchor not found in ${file}: ${oldText.slice(0, 120)}`);
  write(file, content.replace(oldText, newText));
}
function replaceRegex(file, regex, replacement) {
  const content = read(file);
  if (!regex.test(content)) throw new Error(`Regex patch anchor not found in ${file}: ${regex}`);
  write(file, content.replace(regex, replacement));
}

write('src/services/vehicle.warmup.js', `const { decodeVinNhtsa } = require('./vin');
...
write('src/routes/vehicle.js', `const express = require('express');
...
replaceOnce('src/services/vehicle.evidence.js',
...
replaceOnce('src/routes/estimate.js',
...
replaceRegex('src/routes/estimate.js', /async function buildEvidenceVehicle[\s\S]*?function getRelevantTsbs\(vehicleEvidence\) \{[\s\S]*?\n\}\n\n/, '');
replaceOnce('src/routes/estimate.js',
...
replaceOnce('src/routes/diagnose.js',
...
replaceRegex('public/index.html', /        async function decodeVin\(vin, yearEl, makeEl, modelEl, trimEl\) \{[\s\S]*?        \}\n\n        document\.getElementById\('btnDecodeVin'\)/, `        function storeActiveVehicle(vehicle) {
...
`,
`                const diagVin = document.getElementById('diagVin').value.trim();
                const payload = {
                    vin: diagVin,
...
write('scripts/test-evidence-selection.js', `const assert = require('assert');
...
write('scripts/test-vehicle-profile-merge.js', `const assert = require('assert');
...
console.log('VIN evidence warmup patch applied.');
