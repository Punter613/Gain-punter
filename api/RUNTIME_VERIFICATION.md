...
Navigation is intentionally broader because it is not evidence. A known DTC may use stored structural shortcuts such as Powertrain Management, Ignition, Fuel Delivery, Air Intake, Emission Control, or Testing/Inspection. The already-resolved Repair & Diagnosis root also contributes a deterministic `ALL Diagnostic Trouble Codes (DTC)` index URL as a navigation-only shortcut. That synthetic index never claims DTC coverage and never becomes evidence by construction; it merely gives the bounded worker a direct place to discover the manual's own code-chart links. Only fetched page identity/text can satisfy the evidence guard.
