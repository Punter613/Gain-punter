...
Navigation is intentionally broader because it is not evidence. A known DTC may also use deterministic structural shortcuts such as the applicable Powertrain Management, Ignition, Fuel Delivery, Air Intake, Emission Control, or Testing/Inspection branch to reach a more specific page faster. These structural links are routing hints only: they do not claim DTC coverage, do not become a manual reference merely by being selected, and cannot satisfy the final evidence guard. Only the fetched page's visible source text can establish DTC coverage. For multi-DTC requests, stored navigation candidates are selected round-robin across each requested DTC's routing family before remaining seed slots are filled by global score, preventing one code family from monopolizing the bounded probe. Multi-DTC probes cap structural entry points at roughly three per requested DTC (six for P0300+P0171) so the bounded page budget retains room to descend from branch roots into actual diagnostic/test pages.

A stored-navigation seed represents a **bounded subtree probe**, not a one-page probe. Descendants discovered beneath a selected seed retain the seed marker, stay under the seed fetch timeout/crawl budget, and may contribute to seed early-stop only when the descendant page's own visible title/headings/path/body satisfies the DTC applicability matcher. Structural ancestry alone never counts as evidence. This lets the probe stop as soon as the fetched subtree collectively proves every requested DTC instead of burning the probe window on branch roots while ignoring qualifying child pages.
...
Stored-navigation work is now explicitly two-phase. The seed phase gets its own bounded worker invocation with a strict ~4.5-second crawl budget and bounded fetches/pages. Its returned pages are revalidated through the same visible-source DTC gate before they can be accepted. The crawl budget starts only after manual-path resolution inside the worker, so the short-probe parent wall separately allows the resolver's existing <=10-second budget plus a small startup/serialization margin; this does **not** widen the 4.5-second crawl itself. Normal 20-second retrieval keeps the existing 30-second hard wall unchanged. If the seed phase finds qualifying evidence, that focused result is persisted and returned without walking the full manual tree. If it does not, the probe is discarded as evidence and only then does the existing isolated ~20-second targeted root crawl run as fallback.

A seeded network fetch is not mislabeled as a cache hit. When the bounded probe succeeds, its focused result is persisted normally so the next identical request can become an exact revalidated cache hit. When the probe fails, the full fallback is labeled and persisted according to the normal live-targeted path.

The reuse-specific CI canary uses the 2008 Kia Sorento 3.8L 4WD (`KNDJC736385765089`) with P0300/P0171 plus deceleration/full-lock context from the production latency case; it requires at least one focused manual reference, no manual timeout, <=5 seconds for the warm manual source, and a persisted cache hit on the repeat request. Because that lane is multi-DTC, the warm stored result must survive the collective P0300+P0171 coverage guard; partial single-code reuse will fall through and fail the latency/cache assertions instead of being accepted. Render logs should demonstrate local page reuse or a bounded stored-navigation probe before any full ~20-second manual-tree walk.
