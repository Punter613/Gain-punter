...
Stored-navigation work is now explicitly two-phase. The seed phase gets its own bounded worker invocation (default about 4.5 seconds of crawl budget and bounded fetches/pages). Its returned pages are revalidated through the same visible-source DTC gate before they can be accepted. The crawl budget starts after manual-path resolution inside the worker, so the parent hard wall separately allows up to about 7 seconds of bounded resolver/thread-startup/serialization headroom in addition to the 4.5-second crawl budget. The normal 20-second crawl / 30-second hard wall remains unchanged because the same resolver margin still stays below that existing 30-second ceiling. If the seed phase finds qualifying evidence, that focused result is persisted and returned without walking the full manual tree. If it does not, the probe is discarded as evidence and only then does the existing isolated ~20-second targeted root crawl run as fallback.
