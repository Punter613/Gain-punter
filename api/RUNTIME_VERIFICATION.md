...
Navigation is intentionally broader because it is not evidence. A known DTC may use stored structural shortcuts such as Powertrain Management, Ignition, Fuel Delivery, Air Intake, Emission Control, or Testing/Inspection. The already-resolved Repair & Diagnosis root may also contribute a deterministic `ALL Diagnostic Trouble Codes (DTC)` index URL as a navigation-only shortcut. That synthetic index is enabled only when a compatible stored row already contains a retained navigation map; ordinary single-DTC fallback keeps its prior behavior unless it is running the dedicated bounded per-code probe. The synthetic index never claims DTC coverage and never becomes evidence by construction; it merely gives the bounded worker a direct place to discover the manual's own code-chart links. Only fetched page identity/text can satisfy the evidence guard.

For requests with two to four resolved DTCs, the cold navigation path probes each requested DTC **independently and concurrently** using only that code plus its deterministic meaning vocabulary. Each per-code worker uses the two strongest navigation entry points by default—normally the DTC index plus one stored structural branch—so its bounded page/time budget is spent descending toward diagnostic pages rather than sampling many branch roots. The returned per-code manual pages are merged, deduplicated, and then revalidated once against the **original multi-DTC intent**. The merged result is persisted/returned only if that final visible-source set collectively covers every requested code. If any code-specific probe fails, lacks seeds, or the merged result remains incomplete, SKSK fails closed to the existing full mixed-context live crawl. Network probe results remain `fromCache:false`; only a subsequent exact revalidated request may become a cache hit. The code fan-out is capped at four requested DTCs.
