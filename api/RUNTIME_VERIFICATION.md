...
Stored-navigation work is now explicitly two-phase. The seed phase gets its own bounded worker invocation (default about 4.5 seconds of crawl budget, bounded fetches/pages, and a tighter worker hard timeout). Its returned pages are revalidated through the same visible-source DTC gate before they can be accepted. If the seed phase finds qualifying evidence, that focused result is persisted and returned without walking the full manual tree. If it does not, the probe is discarded as evidence and only then does the existing isolated ~20-second targeted root crawl run as fallback. This prevents a structural seed subtree from silently expanding into another full crawl while still preserving the original cold-retrieval behavior when shortcuts fail.
