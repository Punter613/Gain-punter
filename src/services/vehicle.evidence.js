...
async function getJson(url, timeoutMs = 8000) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: { Accept: 'application/json', 'User-Agent': 'SKSK-ProTech/1.0 vehicle-evidence' }
    });
...
function buildKnownIssues(complaints) {
  ...
  for (const complaint of complaints) {
    ...
    if (!groups.has(key)) {
      groups.set(key, {
        system,
        component: clean(complaint.components) || 'Unspecified component',
        reports: 0,
        examples: []
      });
    }
  ...
  return [...groups.values()]
    ...
    .slice(0, 12)
    .map((item, index) => ({ ...item, rank: index + 1, source: 'NHTSA ODI complaints' }));
}
...
async function scrapeNhtsa(vehicle) {
  ...
  const recalls = recallsResult.status === 'fulfilled'
  ...
  return {
    recalls: recalls.slice(0, 25).map(item => ({
      campaignNumber: item.NHTSACampaignNumber || item.CampaignNumber || '',
...
async function collectVehicleEvidence(vehicle) {
  if (!vehicle?.make || !vehicle?.model || !vehicle?.year) {
    return { available: false, error: 'Vehicle year, make, and model are required' };
  }
  ...
  const result = {
    ...
    tsbs: { references: [], status: 'not_available_from_public_nhtsa_api' },
    recalls: [],
    knownIssues: [],
    sources: [],
    errors: []
  ...
  if (manualResult.status === 'fulfilled') {
    ...
    result.errors.push(`LEMON: ${manualResult.reason?.message || 'scrape failed'}`);
  }
  ...
  if (nhtsaResult.status === 'fulfilled') {
    ...
    result.errors.push(`NHTSA: ${nhtsaResult.reason?.message || 'lookup failed'}`);
  }
